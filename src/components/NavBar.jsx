  ...
  return (
    <div className="flex flex-row items-center justify-between text-center bg-white px-8 py-5 lg:px-[20rem] w-screen sticky shadow-md top-0 z-50">
      ...
      <div className="hidden lg:flex space-x-10">
        {menuItems.map((item) => (
          <Link to={item.link} key={item.id}>
            <p
              className={`${
                '/' + item.link === location.pathname ? 'text-blue-500' : 'text-black'
      ...
      <div className=' lg:hidden cursor-pointer font-extrabold text-2xl border-1 border-[#A61D37] hover:bg-[#d40d32]/20 rounded-full p-2 transition duration-300 ease-in'>
        {
          !isOpen ? (
            <div className='transition duration-400 ease-in-out' onClick={() => setIsOpen(true)}><AiOutlineMenu /></div>
          ):(
            <div className='transition duration-400 ease-in-out' onClick={() => setIsOpen(false)}> <AiOutlineClose /> </div>
          )
        }
      </div>
      {/* <div className={`fixed inset-0 z-40  transform transition-transform duration-300 ease-in ${isOpen ? 'block' : 'hidden'}`}> */}
      <div
        className={` ${isOpen ? 'fixed inset-0 bg-black opacity-80 transition duration-400 ease-in-out':'hidden'} `}
          ...
          <Link to={item.link} key={item.id}>
            <p
              onClick={() => 
                setIsOpen(false)}
              className={`${
                '/' + item.link === location.pathname ? 'text-white' : 'text-white/60'
